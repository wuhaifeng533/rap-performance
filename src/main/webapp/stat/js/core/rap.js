...
(function() {
    ...
    p.findNextParameter = function(actionId, parameterId) {
        ...
        if (action) {
            var requestParameterList = action.requestParameterList,
                ...
                performanceParameterList = action.performanceParameterList,
                performanceParameterListNum = performanceParameterList.length;
            ...
            for (i = 0; i < performanceParameterListNum; i++) {
                p = performanceParameterList[i];
                if (p.id == parameterId) {
                    if (i + 1 < performanceParameterListNum) {
                        return performanceParameterList[i + 1].id;
                    }
                    return -1;
                }
            }
    ...
    p.addAction = function(obj, addExisted, isCopy) {
        ...
        if (!addExisted || addExisted === 'mount') {
            ...
            */
            ...
            for (i = 0; i < obj.performanceParameterList.length; i++) {
                recurUpdateParamId(obj.performanceParameterList[i]);
            }
        }
    ...
    /**
     * wuhaifeng
     * add performance parameter
     */
    p.addPerformanceParameter = function(actionId) {
        var pNew = newParameter();
        this.getAction(actionId).performanceParameterList.push(pNew);
        return pNew.id;
    };
            ...
            for (var j = 0; j < pageListCount; j++) {
                ...
                for (var k = 0; k < actionListCount; k++) {
                    var action = actionList[k],
                        ...
                        performanceParameterList = action.performanceParameterList,
                        performanceParameterListCount = performanceParameterList.length;
                    ...
                    for (l = 0; l < performanceParameterListCount; l++) {
                        parameter = performanceParameterList[l];
                        if (parameter.id == id) {
                            performanceParameterList.splice(l, 1);
                            return 0;
                        }
                        if (removeParameterRecursively(parameter, id) === 0) {
                            return 0;
        ...
        for (var i = n -1; i > -1; i--) {
            ...
            for (var j = pageListCount -1; j > -1; j--) {
                ...
                for (var k = actionListCount -1; k > -1; k--) {
                    ...
                    for (l = performanceParameterListCount -1; l > -1; l--) {
                        itemL = performanceParameterList[l];
                        if (this.parameterAutoCompleteRecursively(itemL, param)) {
                            return true;
                        }
                    }
...
(function() {
    ...
    var ws       = rap.workspace,       // workspace module
        ...
        TEMPLATE = {            // static template
            ...
            "PERFORMANCE_BEGIN"                : "<h2>性能基准数据</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-identifier\">基准项</td><td class=\"head-name\">基准值</td><td class=\"head-type\">类型</td><td class=\"head-remark\">备注</td></tr>",
            "PERFORMANCE_BEGIN_EDIT"           : "<h2>性能基准数据</h2><table class=\"table-a\"><tr class=\"head\"><td class=\"head-expander\"></td><td class=\"head-op\">OP</td><td class=\"head-identifier\">基准项</td><td class=\"head-name\">基准值</td><td class=\"head-type\">类型</td><td class=\"head-remark\">备注</td></tr>",
            "PERFORMANCE_END"                  : "</table>",
            "PERFORMANCE_PARAMETER_ADD_BUTTON" : "<div class='btns-container'><a href=\"#\" class=\"btn btn-info btn-xs\" onclick=\"ws.addParam('performance'); return false;\"><i class='glyphicon glyphicon-plus'></i>添加参数</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href\"#\" class='btn btn-default btn-xs' onclick=\"ws.importJSON(); return false;\"><i class='glyphicon glyphicon-transfer'></i>导入JSON</a></div>",
    ...
    ws.addParam = function(type, parentParamId) {
        ...
        if (type == "child") {
            ...
            newParamId = p.addPerformanceParameter(_curActionId);
        } else {
    ...
    */
    ...
    //wuhaifeng 快存功能，触发ajax post请求 checkin.do
    ...
    function workspaceSearch(key, ev) {
        ...
        function getAHtml(a) {
            var str = "",
                ...
                performanceParameterList = a.performanceParameterList,
                requestParameterListNum = requestParameterList.length,
                responseParameterListNum = responseParameterList.length,
                performanceParameterListNum = performanceParameterList.length,
                p, i,
            ...
            sortParams(performanceParameterList);
            ...
            str += _isEditMode ? TEMPLATE.PERFORMANCE_BEGIN_EDIT : TEMPLATE.PERFORMANCE_BEGIN;
            for (i = 0; i < performanceParameterListNum; i++) {
                p = performanceParameterList[i];
                str += getPTRHtml(p);
            }
            str += TEMPLATE.PERFORMANCE_END;

            if (_isEditMode) {
                str += TEMPLATE.PERFORMANCE_PARAMETER_ADD_BUTTON;
            }
